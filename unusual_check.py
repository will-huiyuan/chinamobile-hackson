...
def high_freq_op_and_work_overtime(operations, output_file, limit_frequency=5):
    ...
    for operation in operations:
        if len(operation[1]) > limit_frequency and not high_freq:
            output_file.append([operation[1].iloc[0, 2], operation[1].iloc[0, 0], "业务高频访问", operation[0]])
            high_freq = True
        if operation[0] == "用户登陆" and not_in_work_time(operation[1].iloc[0, 0]):
            for _ in range(len(operation[1])):
                output_file.append([operation[1].iloc[0, 2], operation[1].iloc[0, 0], "非工作时间访问", operation[0]])
...
def unusual_login(timed_DF, output_file, limit_frequency=8, limit_ips=2):
    ...
    if len(timed_DF) >= limit_frequency:
        ...
        for i in range(len(timed_DF)):
            output_file.append(
                [timed_DF.iloc[i, 2], timed_DF.iloc[i, 0], "登录异常", f"ip异常 {i},{len(timed_DF['源IP'].unique())}"])
